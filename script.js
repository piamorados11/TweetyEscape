...
pipeImgTop.src = 'https://i.postimg.cc/VL2kdHcr/tree.png';

const pipeImgBottom = new Image();
pipeImgBottom.src = 'https://i.postimg.cc/VL2kdHcr/tree.png';
...
function drawMenu() {
  ...
  ctx.drawImage(logoImg, canvas.width / 3.3 - 150, canvas.height / 2 - 200, 400, 300);

  ctx.fillStyle = 'white';
  ctx.font = 'bold 48px "Lucida Handwriting", cursive';
  ctx.fillText('Tweety Escape', canvas.width / 3 - 150, canvas.height / 1.1 - 60);
...
function drawHowToPlay() {
  ...
  ctx.drawImage(instructionImg, canvas.width / 2 - 196, canvas.height / 2 - 200, 400, 400);
  ...
  ctx.fillStyle = 'black';
  ctx.font = 'bold 34px "Pixelify Sans"';
  ctx.fillText('Back', canvas.width / 1.23 - 100, canvas.height / 1.35 + 60, 100, 50);
}

function drawScore() {
  ctx.fillStyle = 'aquamarine';
  ctx.font = '58px Arial bold';
...
function drawBestScore() {
  ctx.fillStyle = 'black';
  ctx.font = '24px Arial';
  ctx.fillText('Best: ' + bestScore, 15, 65);
}
...
function drawGameOver() {
  ...
  // Draw "GAME OVER" text
  ctx.fillStyle = 'red';
  ctx.font = 'bold 78px "Pixelify Sans"';
  ctx.fillText('GAME OVER', canvas.width / 2 - 225, canvas.height / 2 - 120);

  // Draw score text
  ctx.fillStyle = 'white';
  ctx.font = 'bold 48px Arial';
  ctx.fillText('Score: ' + score, canvas.width / 2 - 120, canvas.height / 2 + 10);
  if (newBest) {
    ctx.fillStyle = 'yellow';
    ctx.fillText('New Best!', canvas.width / 2 - 100, canvas.height / 2 + 60);
  }
...
function generatePipes() {
  if (pipes.length > 0) {
    ...
    if (canvas.width - lastPipe.x > 100) {
      pipes.push(new Pipe());
    }
  } else {
    pipes.push(new Pipe());
